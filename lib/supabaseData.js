...
export async function readCollection(name) {
  ...
  if (data) return Array.isArray(data.value) ? data.value : []

  // First deploy convenience: seed Supabase from existing local JSON if available.
  const localValue = readLocalCollection(name)
  await writeCollection(name, localValue)
  return localValue
}
